...
class BasicLayer(nn.Module):
    def __init__(self, network_depth, dim, depth, num_heads, mlp_ratio=4.,
        ...
        if attn_loc == 'last':
            ...
            use_attns = [(depth - attn_depth) // 2 <= i < (depth + attn_depth) // 2 for i in range(depth)]
...
class DehazeFormer(BaseModule):
    def __init__(self, in_chans=3, out_chans=4, window_size=8,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(DehazeFormer, self).__init__(init_cfg, **kwargs)
    ...
    def forward_features(self, x):
        x = self.patch_embed(x)  # [B, 24, 512, 512]
        x = self.layer1(x)  # [B, 24, 512, 512]
        skip1 = x

        x = self.patch_merge1(x)  # [B, 48, 256, 256]
        x = self.layer2(x)  # [B, 48, 256, 256]
        skip2 = x

        x = self.patch_merge2(x)  # [B, 96, 128, 128]
        x = self.layer3(x)  # [B, 96, 128, 128]
        temp3 = x
        x = self.patch_split1(x)  # [B, 48, 256, 256]

        x = self.fusion1([x, self.skip2(skip2)]) + x  # [B, 48, 256, 256]
        x = self.layer4(x)  # [B, 48, 256, 256]
        temp4 = x
        x = self.patch_split2(x)  # [B, 24, 512, 512]

        x = self.fusion2([x, self.skip1(skip1)]) + x  # [B, 24, 512, 512]
        x = self.layer5(x)  # [B, 24, 512, 512]
        temp5 = x
        x = self.patch_unembed(x)  # [B, 4, 512, 512] (K, B)
        return x, skip1, skip2, temp3, temp4, temp5

    def forward(self, x):
        H, W = x.shape[2:]
        x = self.check_image_size(x)

        feat, l1, l2, l3, l4, l5 = self.forward_features(x)
...
class DehazeFormerT(DehazeFormer):
    def __init__(self, **kwargs):
        super(DehazeFormerT, self).__init__(
            embed_dims=[24, 48, 96, 48, 24],
            mlp_ratios=[2., 4., 4., 2., 2.],
            depths=[4, 4, 4, 2, 2],
            num_heads=[2, 4, 6, 1, 1],
...
# @MODELS.register_module()
class DehazeFormerS(DehazeFormer):
    def __init__(self, **kwargs):
        super(DehazeFormerS, self).__init__(
            embed_dims=[24, 48, 96, 48, 24],
            mlp_ratios=[2., 4., 4., 2., 2.],
            depths=[8, 8, 8, 4, 4],
            num_heads=[2, 4, 6, 1, 1],
...
class DehazeFormerB(DehazeFormer):
    def __init__(self, **kwargs):
        super(DehazeFormerB, self).__init__(
            embed_dims=[24, 48, 96, 48, 24],
            mlp_ratios=[2., 4., 4., 2., 2.],
            depths=[16, 16, 16, 8, 8],
            num_heads=[2, 4, 6, 1, 1],
