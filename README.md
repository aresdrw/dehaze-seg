...
```
**HazyUAVid**: Download all image and label packages from [BaiduNetdisk](https://acdc.vision.ee.ethz.ch/) Extraction code: rsdl and extract them to `datasets/foggy_uavid_for_train`.

**HazyUDD**: Download all image and label packages from [BaiduNetdisk](http://www.urbansyn.org/#loaded) Extraction code: rsdl and extract them to `datasets/foggy_udd_for_train`.
...
```
## Pretraining Weights
* **Download:** Download pre-trained weights from [BaiduNetdisk](https://dl.fbaipublicfiles.com/dinov2/dinov2_vitl14/dinov2_vitl14_pretrain.pth) for testing. Place them in the project directory without changing the file name.
## Dehaze Inference
  Run the evaluation:
  ```
  cd Dehaze_Seg
  python tools/dehaze_inference.py --config ./work_dirs/./configs/dinov2/rein_dinov2_mask2former_512x512_bs1x4.py checkpoints/dinov2_rein_and_head.pth --backbone dinov2_converted.pth
  ```
